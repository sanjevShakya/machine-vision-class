...
public:
    ...
    void runSfM()
    {
        extractFeatures();
        matchFeatures();
        std::cout << "Building track" << endl;
        buildTracks();
        std::cout << "Building track finish" << endl;
        std::cout << "Reconstrcut from track" << endl;
        reconstructFromTracks();
        std::cout << "Reconstrcut from finish" << endl;
        std::cout << "Visualize flag " << visualize << endl;
        if (false)
        {
            visualize3D();
        }
        ...
        if (true)
        {
            // cv::CV_LOG_INFO(TAG, "Save point cloud to: " + saveCloud);
...
private:
    ...
    {
        // cv::CV_LOG_INFO(TAG, "Finding images in " + dir);
        ...
        // cv::CV_LOG_INFO(TAG, "Found " + std::to_string(imagesFilenames.size()) + " images");

        // cv::CV_LOG_INFO(TAG, "Reading images...");
        for (const auto &i : imagesFilenames)
        {
            // cv::CV_LOG_INFO(TAG, i);
            images[i] = imread(i);
    ...
    {
        // cv::CV_LOG_INFO(TAG, "Extract Features");
        ...
        for (const auto &i : imagesFilenames)
        {
            ...
            // cv::CV_LOG_INFO(TAG, "Found " + to_string(keypoints[i].size()) + " keypoints in " + i);
    ...
    void matchFeatures()
    {
        // cv::CV_LOG_INFO(TAG, "Match Features");
        ...
        for (size_t i = 0; i < imagesFilenames.size() - 1; ++i)
        {
            for (size_t j = i + 1; j < imagesFilenames.size(); ++j)
            {
                ...
                {
                    // cv::CV_LOG_INFO(TAG, "Final match '" + imgi + "'->'" + imgj + "' has less than " + to_string(PAIR_MATCH_SURVIVAL_RATE) + " inliers from orignal. Skip");
                    continue;
                }

                matches[make_pair(imgi, imgj)] = final;

                // cv::CV_LOG_INFO(TAG, "Matching " + imgi + " and " + imgj + ": " + to_string(final.size()) + " / " + to_string(match.size()));
    ...
    void buildTracks()
    {
        // cv::CV_LOG_INFO(TAG, "Build tracks");
        ...
        Filtered gFiltered(g, keep_all{}, [&g](Vertex vd)
        ...
        // cv::CV_LOG_INFO(TAG, "Total number of components found: " + to_string(components.size()));
        // cv::CV_LOG_INFO(TAG, "Number of good components: " + to_string(goodComponents.size()));
        ...
        // cv::CV_LOG_INFO(TAG, "Average component size: " + to_string((float)accum / (float)(goodComponents.size())));
    ...
    {
        // cv::CV_LOG_INFO(TAG, "Reconstruct from " + to_string(tracks[0].cols) + " tracks");
        ...
        cout << K_ << endl; 
        // cv::CV_LOG_INFO(TAG, "Reconstruction: ");
        // cv::CV_LOG_INFO(TAG, "Estimated 3D points: " + to_string(points3d.size()));
        // cv::CV_LOG_INFO(TAG, "Estimated cameras: " + to_string(Rs.size()));
        // cv::CV_LOG_INFO(TAG, "Refined intrinsics: ");
        // cv::CV_LOG_INFO(TAG, K_);

        if (Rs.size() != imagesFilenames.size())
        {
            // CV_LOG_ERROR(TAG, "Unable to reconstruct all camera views (" + to_string(imagesFilenames.size()) + ")");
            std::cout << "ERROR: Unable to reconstruct all camera views (" + to_string(imagesFilenames.size()) << endl;
            return false;
        }
        ...
        if (tracks[0].cols != points3d.size())
        {
            ...
            cout << "Warning unable to reconstruct all tracks" << endl;
        }
    ...
    void visualize3D()
    {
        // cv::CV_LOG_INFO(TAG, "Visualize reconstruction");
        ...
        // cv::CV_LOG_INFO(TAG, "Press 'q' to close ... ")
